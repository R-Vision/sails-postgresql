...
if (fs.existsSync(__dirname + '/cache.json')) {
  CACHE = JSON.parse(fs.readFileSync(__dirname + '/cache.json', 'utf8').toString());
}
...
module.exports = (function() {
  ...
  var adapter = {
    ...
    registerConnection: function(connection, collections, cb) {
      ...
      async.map(_.keys(collections), function(collName, cb){
        var md5 = crypto.createHash('md5')
          .update(JSON.stringify(collections[collName].definition))
          .digest("hex");
        self.describe(connection.identity, collName, md5, cb);
      }, cb);
    },
    ...
    // Describe a table
    describe: function(connectionName, table, md5, cb) {
      if (CACHE[md5]) {
        return cb(null, CACHE[md5]);
      }
    ...
    define: function(connectionName, table, definition, cb) {
      ...
      var describe = function(err, result) {
        ...
        adapter.describe(connectionName, table.replace(/["']/g, ''), md5, cb);
      };
